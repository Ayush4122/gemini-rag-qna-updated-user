...
# Function to render PDF preview using PyMuPDF
def create_pdf_preview(file):
    try:
        # Need to convert to BytesIO since file is a Streamlit UploadedFile
        ...
        # Create a slider for page navigation (only if more than 1 page)
        if total_pages > 1:
...
if input_method == "Website URL":
    ...
    if uploaded_file:
        ...
        # Create tabs for preview and Q&A
        preview_tab, qa_tab = st.tabs(["Document Preview", "Ask Questions"])
        ...
        with preview_tab:
            ...
            if file_extension == 'pdf':
                ...
                # We need to seek to the beginning of the file before reading it
                uploaded_file.seek(0)
                create_pdf_preview(uploaded_file)
                # Reset file position for processing
                uploaded_file.seek(0)
                # Process content for Q&A
                content = read_pdf(uploaded_file)
                
            elif file_extension == 'docx':
                st.write("DOCX Preview (Text Only):")
                content = read_docx(uploaded_file)
        ...
        question = st.text_input("Ask a question about the content:")
        if question:
            st.warning("Please upload a document or enter a website URL first.")
